      ...
      // ..if not, let's initialize.
        ...
        this.twitter = {
          ...
          apiURL: function () {
			...
			// console.log("QUERY: " + protocol + '//' + baseURL + endpoint + '.json?' + queryString + '&callback=?');
          ...
          parseTweet: function (json) {
            var tweet = {
              ...
              text:       json.text,
			  screen_name:      json.user.from_user,
          ...
          toP5: function (tweet) {
            return {
              id: tweet.id,
              profileName: tweet.from_user, /// screen name does not seem to work
